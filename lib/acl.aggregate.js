...
module.exports = {
            ...
            async handler(ctx) {
                switch (ctx.params.event) {
                    case "groups.user.joined":
                        ...
                        await this.addMember({ groupId: ctx.params.payload.groupId, userId: ctx.params.payload.userId, role:ctx.params.payload.role, core:ctx.params.payload.core });
                        break;
                    ...
                    case "account.created":
                        if ( !ctx.params.payload.groupId || !ctx.params.payload.accountId ) return false;
                        await this.addAccount({ groupId: ctx.params.payload.groupId, accountId: ctx.params.payload.accountId });
                        break;
                    case "account.deleted":
                        if ( !ctx.params.payload.groupId || !ctx.params.payload.accountId ) return false;
                        await this.deleteAccount({ groupId: ctx.params.payload.groupId, accountId: ctx.params.payload.accountId });
                        break;
                }
    ...
    methods: {
        ...
        async addAccount({ groupId, accountId } = {}) {
            let params = {
                groupId,
                accountId
            };
            let statement = "MERGE (a:Account { uid: {accountId}, ownerId: {groupId} }) ";
            statement += ";";
            await this.run(statement, params);
            this.logger.debug(`Account ${accountId} added to group ${groupId}`, { accountId, groupId });
            return true;
        },
        ...
        async deleteAccount({ groupId, accountId } = {}) {
            ...
            let statement = "MATCH (a:Account { uid: {accountId} ownerId: {groupId} }) ";
            statement += "DETACH DELETE a ";
            statement += ";";
            await this.run(statement, params);                    
            this.logger.debug(`Account ${accountId} of group ${groupId} deleted`, { accountId, groupId });
            return true;
        },
