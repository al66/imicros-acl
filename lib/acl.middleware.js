    ...
    */
    ...
    function isAuthorized( { ctx = null, result = {}, abort = false } = {} ) { 
        ...
        if (!user) {
            ...
            // no access at all
        if (!acl) {
            ctx.broker.logger.debug("not authorized access", { user: user.userId, action: ctx.action } );
    ...
    return {
        ...
        localAction(next, action) {
            return async function(ctx) {
                let serviceToken = _.get(ctx,"meta.serviceToken",null);
                if (ctx.meta.acl) {
                ...
                // granted service
                if (serviceToken && !ctx.meta.acl) {
                    // TODO: verify service token and grants!!
                    ctx.meta.acl = {
                ...
                ctx.meta.ownerId = _.get(ctx.meta,"acl.ownerId",ctx.meta.ownerId);
                ...
                if (action.acl === "before" || action.acl === "always"  || action.acl === true ) {
                    await isAuthorized( { ctx: ctx, action: action.name, params: ctx.params, abort: true } );
                }
                
                if (action.acl === "after" || action.acl === "always") {
                    const res = await next(ctx);
                    await isAuthorized( { ctx: ctx, action: action.name, params: ctx.params, result: res, abort: true } );
                    return res;
                } else {
                    return next(ctx);
                }
