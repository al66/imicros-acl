    ...
    actions: {
        ...
        requestAccess: {
            ...
            async handler(ctx) {
                ...
                // access with group account
                if (ctx.meta.serviceToken && ctx.meta.accountId) {
                    ...
                    // Is account exiting and owned by group?
                    let statement = "MATCH (a:Account { uid: {accountId}, ownerId: {groupId} }) ";
                    statement += "RETURN a.uid AS accountId, a.ownerId AS ownerId ";
                    statement += ";";

                    this.logger.debug("request access for account", { statement: statement, params: params });
                    ...
                    if (result[0]) {
                        ...
                        let options = { subject: ctx.meta.accountId };
